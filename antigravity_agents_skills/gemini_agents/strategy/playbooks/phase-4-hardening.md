...
```
Activate the `evidence-collector` agent for comprehensive system evidence on [PROJECT].
...
```
Activate the `api-tester` agent for complete API regression on [PROJECT].
...
```
Activate the `performance-benchmarker` agent for load testing on [PROJECT].
...
```
Activate the `legal-compliance-checker` agent for final compliance audit on [PROJECT].
...
```
Activate the `test-results-analyzer` agent for quality metrics aggregation on [PROJECT].
...
```
Activate the `workflow-optimizer` agent for process efficiency review on [PROJECT].
...
```
Activate the `infrastructure-maintainer` agent for production readiness on [PROJECT].
...
```
Activate the `reality-checker` agent for final integration testing on [PROJECT].
