...
```
Activate the `studio-producer` agent for strategic portfolio alignment on [PROJECT].
...
```
Activate the `brand-guardian` agent for brand identity development on [PROJECT].
...
```
Activate the `finance-tracker` agent for financial planning on [PROJECT].
...
```
Activate the `ux-architect` agent for technical architecture on [PROJECT].
...
```
Activate the `backend-architect` agent for system architecture on [PROJECT].
...
```
Activate the `ai-engineer` agent for ML system architecture on [PROJECT].
...
```
Activate the `senior-project-manager` agent for task list creation on [PROJECT].
...
```
Activate the `sprint-prioritizer` agent for backlog prioritization on [PROJECT].
