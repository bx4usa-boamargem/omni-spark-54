...
```
Activate the `trend-researcher` agent for market intelligence on [PROJECT DOMAIN].
...
```
Activate the `feedback-synthesizer` agent for user needs analysis on [PROJECT DOMAIN].
...
```
Activate the `ux-researcher` agent for user behavior analysis on [PROJECT DOMAIN].
...
```
Activate the `analytics-reporter` agent for data landscape assessment on [PROJECT DOMAIN].
...
```
Activate the `legal-compliance-checker` agent for regulatory scan on [PROJECT DOMAIN].
...
```
Activate the `tool-evaluator` agent for technology landscape assessment on [PROJECT DOMAIN].
...
```
Activate the `executive-summary-generator` agent to synthesize Phase 0 findings.
