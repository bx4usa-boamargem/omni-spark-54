...
```
Activate the `devops-automator` agent for infrastructure setup on [PROJECT].
...
```
Activate the `infrastructure-maintainer` agent for monitoring setup on [PROJECT].
...
```
Activate the `frontend-developer` agent for project scaffolding on [PROJECT].
...
```
Activate the `backend-architect` agent for API foundation on [PROJECT].
...
```
Activate the `ux-architect` agent for CSS system implementation on [PROJECT].
...
```
Activate the `evidence-collector` agent for Phase 2 foundation verification.
