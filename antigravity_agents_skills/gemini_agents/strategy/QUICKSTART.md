...
```
Activate the `agents-orchestrator` agent in NEXUS-Full mode.
...
```
Activate the `agents-orchestrator` agent in NEXUS-Sprint mode.
...
### Fix a Bug
```
Activate the `backend-architect` agent to investigate and fix [BUG DESCRIPTION].
After fix, Activate the `api-tester` agent to verify the fix.
Then Activate the `evidence-collector` agent to confirm no visual regressions.
```

### Run a Marketing Campaign
```
Activate the `social-media-strategist` agent as campaign lead for [CAMPAIGN DESCRIPTION].
...
```
Activate the `legal-compliance-checker` agent for comprehensive compliance audit.
Scope: [GDPR / CCPA / HIPAA / ALL]
After audit, Activate the `executive-summary-generator` agent to create stakeholder report.
```

### Investigate Performance Issues
```
Activate the `performance-benchmarker` agent to diagnose performance issues.
Scope: [API response times / Page load / Database queries / All]
After diagnosis, Activate the `infrastructure-maintainer` agent for optimization.
DevOps Automator deploys any infrastructure changes.
```

### Market Research
```
Activate the `trend-researcher` agent for market intelligence on [DOMAIN].
Deliverables: Competitive landscape, market sizing, trend forecast.
After research, Activate the `executive-summary-generator` agent for executive brief.
```

### UX Improvement
```
Activate the `ux-researcher` agent to identify usability issues in [FEATURE/PRODUCT].
After research, Activate the `ux-architect` agent to design improvements.
