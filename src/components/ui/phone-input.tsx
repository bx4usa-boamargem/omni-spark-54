...
const COUNTRIES: Country[] = [
  { code: "BR", name: "Brasil", ddi: "55", flag: "🇧🇷", placeholder: "11 99999-9999", minLength: 10, maxLength: 11 },
  { code: "US", name: "Estados Unidos", ddi: "1", flag: "🇺🇸", placeholder: "212 555-1234", minLength: 10, maxLength: 10 },
  { code: "PT", name: "Portugal", ddi: "351", flag: "🇵🇹", placeholder: "912 345 678", minLength: 9, maxLength: 9 },
  { code: "MX", name: "México", ddi: "52", flag: "🇲🇽", placeholder: "55 1234 5678", minLength: 10, maxLength: 10 },
  { code: "AR", name: "Argentina", ddi: "54", flag: "🇦🇷", placeholder: "11 1234-5678", minLength: 10, maxLength: 10 },
  { code: "ES", name: "Espanha", ddi: "34", flag: "🇪🇸", placeholder: "612 345 678", minLength: 9, maxLength: 9 },
  { code: "GB", name: "Reino Unido", ddi: "44", flag: "🇬🇧", placeholder: "7911 123456", minLength: 10, maxLength: 11 },
  { code: "CA", name: "Canadá", ddi: "1", flag: "🇨🇦", placeholder: "416 555-1234", minLength: 10, maxLength: 10 },
];
...
// Validate phone length for country
export function validatePhoneLength(digits: string, country: Country): PhoneValidationResult {
  const length = digits.length;
  
  if (length === 0) {
    return { valid: true, message: "", status: "incomplete" };
  }
  
  if (length < country.minLength) {
    return { 
      valid: false, 
      message: `Mínimo ${country.minLength} dígitos (faltam ${country.minLength - length})`,
      status: "incomplete"
    };
  }
  ...
  if (length > country.maxLength) {
    return { 
      valid: false, 
      message: `Máximo ${country.maxLength} dígitos`,
      status: "invalid"
    };
  }
  
  if (length >= country.minLength && length <= country.maxLength) {
    return { valid: true, message: "Número válido", status: "valid" };
  }
  
  return { valid: false, message: "", status: "incomplete" };
}
...
export function PhoneInput({
  ...
  const validation = React.useMemo(() => {
    return validatePhoneLength(localNumber, country);
  }, [localNumber, country]);
  ...
  const getBorderClass = () => {
    if (!showValidation || localNumber.length === 0) return "";
    if (validation.status === "valid") return "border-green-500 focus-visible:ring-green-500";
    if (validation.status === "invalid") return "border-destructive focus-visible:ring-destructive";
    return "";
  };
    ...
    <div className={cn("space-y-1", className)}>
      <div className="flex gap-2">
        ...
        <Input
          ...
          disabled={disabled}
          className={cn("flex-1 font-mono", getBorderClass())}
        />
      </div>
      
      {/* Validation Message */}
      {showValidation && localNumber.length > 0 && validation.message && (
        <p className={cn(
          "text-xs",
          validation.status === "valid" ? "text-green-600" : "text-muted-foreground",
...
// Export for external form validation
export { COUNTRIES, type Country, type PhoneValidationResult };
